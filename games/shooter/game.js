...
class GameScene extends Phaser.Scene {
  ...
  onBulletHitEnemy(bullet, enemy) {
    ...
    if (Math.random() < 0.2) {
      ...
      pu.powerupType = type;
      pu.label = this.add.text(enemy.x, enemy.y, POWERUP_LABELS[type], { fontSize: '16px', fontFamily: 'Arial' })
        .setOrigin(0.5).setDepth(6);
      pu.on('destroy', () => pu.label?.destroy());
      // Drift downward
      pu.setVelocityY(40);
  ...
  update(time) {
    ...
    // Sync powerup labels to their sprites
    this.powerups.getChildren().forEach(pu => {
      if (pu.label) pu.label.setPosition(pu.x, pu.y);
    });
